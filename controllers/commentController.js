...
    await recordGeoActivity(req, 'comment', entity);
    await recordActivityHit(`activity:comments:${req.cid}`, 'added', entity);
...
    await recordGeoActivity(req, 'reply', entity);
    await recordActivityHit(`activity:replies:${req.cid}`, 'added', entity);
...
      await profileService.deleteProfileCache(cid, author);
      // MODIFICACIÓN: Pasamos entity a recordActivityHit
      await recordActivityHit(`activity:likes:${cid}`, 'removed', entity);
    } else {
...
      await recordGeoActivity(req, 'like', entity);
      await recordActivityHit(`activity:likes:${cid}`, 'added', entity);
...
    const activityType = commentDoc.parent ? 'replies' : 'comments';
    // MODIFICACIÓN: Pasamos entity a recordActivityHit
    await recordActivityHit(`activity:${activityType}:${cid}`, 'deleted', commentDoc.entity);
...
  try {
// ... (resto del código sin cambios en este método)
    const { comment } = req.params;
